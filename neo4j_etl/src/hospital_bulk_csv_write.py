...
def get_csv_path(filename: str) -> str:
    ...
    LOGGER.info(f"Getting path to {filename}.csv")
    neo4j_etl = getenv("NEO4J_ETL_URL", "http://neo4j_etl:8000")
    local_path = f"{neo4j_etl}/{filename}.csv"
    path_to_csv = getenv(f"{filename.capitalize}_CSV_PATH", local_path)
    LOGGER.info(f"Path to {filename}.csv: {path_to_csv}")
    return path_to_csv

# Paths to CSV files containing hospital data
HOSPITALS_CSV_PATH = get_csv_path("hospitals")
PAYERS_CSV_PATH = get_csv_path("payers")
PHYSICIANS_CSV_PATH = get_csv_path("physicians")
PATIENTS_CSV_PATH = get_csv_path("patients")
VISITS_CSV_PATH = get_csv_path("visits")
REVIEWS_CSV_PATH = get_csv_path("reviews")
